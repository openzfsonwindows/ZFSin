    ...
    db_lock_type_t dblt, void *tag)
{
	...
	if (db->db_blkid == DMU_BONUS_BLKID) {
		...
		if (err != 0) {
			DB_DNODE_EXIT(db);
			mutex_exit(&db->db_mtx);
			dprintf("%s:%d: dbuf_read_verify_dnode_crypt returned %d\n", __func__, __LINE__, err);
			return (err);
 		}
...
dbuf_read(dmu_buf_impl_t *db, zio_t *zio, uint32_t flags)
{
	...
	if (db->db_state == DB_CACHED) {
		...
		mutex_exit(&db->db_mtx);
		if (err == 0 && prefetch)
			dmu_zfetch(&dn->dn_zfetch, db->db_blkid, 1, B_TRUE, flags & DB_RF_HAVESTRUCT);
		DB_DNODE_EXIT(db);
    ...
    dmu_buf_impl_t **parentp, blkptr_t **bpp, struct dbuf_hold_impl_data *dh)
{
	...
	*bpp = NULL;
	...
	TraceEvent(8, "%s:%d: dn = 0x%p, level = %d, blkid = %llu, fail_sparse = %d, dh = 0x%p\n",
		__func__, __LINE__, dn, level, blkid, fail_sparse, dh);
...
{
	TraceEvent(8, "%s:%d: dh = 0x%p\n", __func__, __LINE__, dh);
	...
	*(dh->dh_dbp) = NULL;
	...
	if (dh->dh_db == NULL) {
		...
		if (dh->dh_fail_uncached) {
			TraceEvent(5, "%s:%d: dh->dh_fail_uncached = %d. Returning ENOENT = %d\n",
				__func__, __LINE__, dh->dh_fail_uncached, ENOENT);
			return (SET_ERROR(ENOENT));
		}
	...
	if (dh->dh_fail_uncached && dh->dh_db->db_state != DB_CACHED) {
		...
		dprintf("%s:%d: dh->dh_db->db_state = %d. Returning ENOENT = %d\n",
			__func__, __LINE__, dh->dh_db->db_state, ENOENT);
		return (SET_ERROR(ENOENT));
	}
