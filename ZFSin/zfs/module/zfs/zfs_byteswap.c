 ...
 */
void
zfs_ace_byteswap(void *buf, uint32_t size, boolean_t zfs_layout)
{
	caddr_t end;
	caddr_t ptr;
...
/* ARGSUSED */
void
zfs_oldacl_byteswap(void *buf, uint32_t size)
{
	int cnt;
...
/* ARGSUSED */
void
zfs_acl_byteswap(void *buf, uint32_t size)
{
	zfs_ace_byteswap(buf, size, B_TRUE);
}

void
zfs_znode_byteswap(void *buf, uint32_t size)
{
	znode_phys_t *zp = buf;
