...

{
    ...
    *DeviceCount = 0;
    ...
    do {
        ...
        if (!NT_SUCCESS(status)) {
            dprintf("DiskCreateFdo: Format symbolic link failed with error: 0x%X\n", status);
            return status;
        }
    ...
    if (!NT_SUCCESS(status)) {
        ...
        dprintf("DiskCreateFdo: Could not create directory - %lx\n", status);
        return(status);
    }
    ...
    if (!NT_SUCCESS(status)) {
		...
		dprintf("DiskCreateFdo: ClassClaim - %lx\n", status);
		return status;
    }
    ...
    if(!NT_SUCCESS(status)) {
        dprintf("DiskCreateFdo - couldn't create name %lx\n", status);
    ...
    if (!NT_SUCCESS(status)) {
        dprintf("DiskCreateFdo: Can not create device object %s\n", deviceName);
        goto DiskCreateFdoExit;
    }
