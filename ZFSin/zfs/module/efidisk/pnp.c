...

{
	...
	TRY {
        ...
        if(!NT_SUCCESS(status)) {
			dprintf("%s: error opening device key: %#08lx\n", __func__, status);
...
//                        status, PhysicalDeviceObject));
        ...
        if(!NT_SUCCESS(status)) {
			dprintf("%s: error opening device key: %#08lx key %p\n", __func__, status, deviceKey);
...
#pragma prefast(suppress:6309, "We don't have QueryRoutine so Context doesn't make any sense")
        ...
        if(!NT_SUCCESS(status)) {
			dprintf("%s: error reading device key: %#08lx\n", __func__, status);
...
//                        status, diskKey, PhysicalDeviceObject));
        ...
        if(!NT_SUCCESS(status)) {
			dprintf("%s: Will %sallow fs to mount on partition zero\n", __func__, (rootPartitionMountable ? "" : "not "));
